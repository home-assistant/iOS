...
class EntitiesGridTemplate {
    ...
    func getGridButtons() -> [CPGridButton] {
        ...
        let entitiesPage = entitiesSorted[(gridPage * CPGridTemplateMaximumItems) ..< min(
            (gridPage * CPGridTemplateMaximumItems) + CPGridTemplateMaximumItems,
            entitiesSorted.count
        )]

        for entity in entitiesPage {
            let item = CPGridButton(
                titleVariants: ["\(entity.attributes.friendlyName ?? entity.entityId) - \(entity.localizedState)"],
        ...
        if entities.count > CPGridTemplateMaximumItems {
            ...
            if gridPage < maxPages {
                barButtons.append(CPBarButton(
                    image: MaterialDesignIcons.pageNextIcon.image(ofSize: CPButtonMaximumImageSize, color: nil),
                    handler: { _ in
                ...
                barButtons
                    .append(CPBarButton(
                        image: UIImage(size: CPButtonMaximumImageSize, color: UIColor.clear),
                        handler: nil
                    ))
            }
            if gridPage > 0 {
                barButtons.append(CPBarButton(
                    image: MaterialDesignIcons.pagePreviousIcon.image(ofSize: CPButtonMaximumImageSize, color: nil),
                    handler: { _ in
...
extension EntitiesGridTemplate: EntitiesStateSubscription {
    public func subscribe() {
        stateSubscriptionToken = Current.api(for: server).connection.caches.states.subscribe { [self] _, cachedStates in
