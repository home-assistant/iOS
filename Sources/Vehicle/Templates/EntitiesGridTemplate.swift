...
class EntitiesGridTemplate {
    ...
    func getGridButtons() -> [CPGridButton] {
        ...
        for entity in entitiesPage {
            let item = CPGridButton(titleVariants: ["\(entity.attributes.friendlyName!) - \(entity.localizedState)"], image: entity.getIcon() ?? MaterialDesignIcons.bookmarkIcon.image(ofSize: entityIconSize, color: nil), handler: { button in
