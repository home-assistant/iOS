...
struct AppMigrationIllustration: View {
    let symbol: SFSymbol
    /// Brand tint for the screens that are asking something of the user, green for the two that
    /// report success and red for the two that report failure — matching the step rows, so a
    /// checkmark means the same thing wherever it appears in the flow.
