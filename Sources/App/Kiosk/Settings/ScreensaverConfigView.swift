...
public struct ScreensaverConfigView: View {
    ...
    private var photoSettings: some View {
        Section {
            ...
            // Album selection based on source type
            if settings.photoSource == .local || settings.photoSource == .all {
                NavigationLink {
                    ...
                    HStack {
                        Text("Device Albums")
                        Spacer()
                        Text(albumSelectionText(for: settings.localPhotoAlbums))
                            .foregroundColor(.secondary)
                    }
                }
            }
            ...
            if settings.photoSource == .iCloud || settings.photoSource == .all {
                NavigationLink {
                    ...
                    HStack {
                        Text("iCloud Albums")
                        Spacer()
                        Text(albumSelectionText(for: settings.iCloudAlbums))
                            .foregroundColor(.secondary)
                    }
                }
            }
            ...
            if settings.photoSource == .local || settings.photoSource == .iCloud {
                Text("Select specific albums to show photos from, or leave empty to use all photos.")
            }
        }
    }

    private func albumSelectionText(for albums: [String]) -> String {
        if albums.isEmpty {
            return "All"
