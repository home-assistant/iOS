...
final class ComplicationListViewModel: ObservableObject {
    ...
    func deleteAll() {
        ...
        // are legacy), so the deletion must also reach the watch through a mirror push — which then
        // asks the watch to re-render the faces the deleted complications were on.
