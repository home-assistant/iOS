...
public extension MagicItem {
    ...
    /// The heavy lifting (token, request, watchdogs) lives in `WatchServiceCall.execute`.
    private func executeViaREST(
        ...
        serviceCall.execute(on: server, logLabel: id, onStep: onStep, completion: completion)
    }
