...
class DebugHelper {
    ...
    static var protectionSpace: URLProtectionSpace?

    // Load your trusted certificates
    static let rootCA = SecCertificateCreateWithData(nil, Bundle.main.data(named: "ReschRootCA")! as CFData)!
    static let intermediateCA = SecCertificateCreateWithData(nil, Bundle.main.data(named: "ReschIntermediateCA")! as CFData)!
    static let trustedCertificates = [DebugHelper.rootCA, DebugHelper.intermediateCA]
}

final class CertificateProvider {
    static func urlCredential(for userCertificate: UserCertificate?, extraCerts: [Any]?) -> URLCredential? {
...
extension Bundle {
    ...
    func data(named name: String) -> Data? {
        guard let path = Bundle.main.path(forResource: name, ofType: "der") else {
            return nil
        }
    ...
    var userCertificate: UserCertificate? {
        guard let path = Bundle.main.path(forResource: "bruno", ofType: "p12"),
              let p12Data = try? Data(contentsOf: URL(fileURLWithPath: path))
        else {
            return nil
        }
        return (p12Data, "bruno")
    }
}
