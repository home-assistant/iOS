...
public extension HealthKitMetric {
    /// Sleep metrics, all read from the one sleep analysis category type: time asleep, time in bed, and
    /// the time in each stage. Each reports the most recent night, so its value holds through the day;
    /// see `HealthKitSleepSummary`.
