...
public struct WatchComplicationGRDB: Codable, FetchableRecord, PersistableRecord {
    ...
    public var complicationData: Data?
    ...
    /// Initialize from Realm WatchComplication for migration
    public init(from realmComplication: WatchComplication) {
        self.identifier = realmComplication.identifier
        self.serverIdentifier = realmComplication.serverIdentifier
        self.rawFamily = realmComplication.value(forKey: "rawFamily") as? String ?? ""
        self.rawTemplate = realmComplication.value(forKey: "rawTemplate") as? String ?? ""
        self.complicationData = realmComplication.value(forKey: "complicationData") as? Data
        self.createdAt = realmComplication.CreatedAt
        self.name = realmComplication.name
        self.isPublic = realmComplication.IsPublic
    }
    ...
    public func renderedValues() -> [RenderedValueType: Any] {
        (Data["rendered"] as? [String: Any] ?? [:])
            .compactMapKeys(RenderedValueType.init(stringValue:))
    }

    public mutating func updateRawRendered(from response: [String: Any]) {
        var data = Data
        data["rendered"] = response
        Data = data
    }

    public func rawRendered() -> [String: String] {
...
extension WatchComplicationGRDB: ImmutableMappable {
    public init(map: ObjectMapper.Map) throws {
        ...
        let template: ComplicationTemplate = try map.value("Template")
        let data: [String: Any] = try map.value("Data")
        let family: ComplicationGroupMember = try map.value("Family")
