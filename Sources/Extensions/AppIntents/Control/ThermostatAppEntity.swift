...
/// A thermostat whose target temperature a spoken command can set.
@available(macOS 13.0, watchOS 9.4, *)
struct ThermostatAppEntity: AppEntity, EntityContextRepresentable {
    static let typeDisplayRepresentation = TypeDisplayRepresentation(name: .init(
        "app_intents.thermostat.entity.name",
        defaultValue: "Thermostat"
    ))

    static let defaultQuery = ThermostatAppEntityQuery()
