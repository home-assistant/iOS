...
struct DimmableLightAppEntityQuery: EntityQuery, EntityStringQuery {
    func entities(for identifiers: [String]) async throws -> [DimmableLightAppEntity] {
        entities().flatMap(\.1).filter { identifiers.contains($0.id) }
    }

    func entities(matching string: String) async throws -> IntentItemCollection<DimmableLightAppEntity> {
        collection(for: entities(matching: string))
    }

    func suggestedEntities() async throws -> IntentItemCollection<DimmableLightAppEntity> {
        collection(for: entities())
    }

    private func collection(
        for entitiesPerServer: [(Server, [DimmableLightAppEntity])]
    ) -> IntentItemCollection<DimmableLightAppEntity> {
        .init(sections: entitiesPerServer.map { server, items in
            .init(.init(stringLiteral: server.info.name), items: items)
        })
    }

    private func entities(matching string: String? = nil) -> [(Server, [DimmableLightAppEntity])] {
        let byServer = ControlEntityProvider(domains: [.light]).getEntitiesExposedToSiri(matching: string)
        let rank = Dictionary(
