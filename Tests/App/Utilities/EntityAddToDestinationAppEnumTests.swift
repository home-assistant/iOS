...
struct EntityAddToDestinationAppEnumTests {
    ...
    /// The rule rather than the answer, since the host decides the idiom: CarPlay quick access is
    /// offered on an iPhone and nowhere else.
    @MainActor
    @Test("CarPlay quick access is an iPhone's alone")
    func carPlayIsAvailableOnAnIPhoneOnly() {
        let previousIsCatalyst = Current.isCatalyst
        defer { Current.isCatalyst = previousIsCatalyst }

        Current.isCatalyst = false
        #expect(
            EntityAddToDestinationAppEnum.carPlay.isAvailable == (UIDevice.current.userInterfaceIdiom == .phone)
        )

        Current.isCatalyst = true
        #expect(!EntityAddToDestinationAppEnum.carPlay.isAvailable)
        #expect(!EntityAddToDestinationAppEnum.appleWatch.isAvailable)
        #expect(EntityAddToDestinationAppEnum.macToolbar.isAvailable)
    }
