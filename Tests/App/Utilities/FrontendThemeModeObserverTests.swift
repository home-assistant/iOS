...
struct FrontendThemeModeObserverTests {
    ...
    @Test func aCoreThatDoesNotKnowTheCommandForgetsTheModeItRemembered() throws {
        try withContext { context in
            context.remember("dark", for: context.server)
            context.observer.observe(server: context.server)

            try #require(context.connection.pendingSubscriptions.first)
                .initiated(.failure(.external(.init(code: "unknown_command", message: "nope"))))

            #expect(context.mode == .automatic)
            #expect(context.remembered(for: context.server) == nil)
        }
    }

    @Test func aFailureThatIsOnlyTransientKeepsTheModeItRemembered() throws {
        try withContext { context in
            context.remember("dark", for: context.server)
            context.observer.observe(server: context.server)

            try #require(context.connection.pendingSubscriptions.first)
                .initiated(.failure(.internal(debugDescription: "connection lost")))
    ...
    @Test func switchingServerReadsThePreferenceFromTheNewOne() throws {
        try withContext { context in
            ...
            #expect(context.mode(for: second) == .light)
    ...
    private final class Context {
        ...
        /// The mode the app would style `server`'s windows with.
        var mode: FrontendThemeMode { mode(for: server) }
