...
struct AppMigrationLinkTests {
    ...
    /// A completion link with no usable count is not one this app wrote. Reading it as "zero
    /// servers" would retire the app being replaced on the strength of a malformed URL.
    @Test(arguments: [
        "homeassistant://migration-complete",
        "homeassistant://migration-complete?servers=",
        "homeassistant://migration-complete?servers=lots",
        "homeassistant://migration-complete?other=2",
    ]) func rejectsAnAcknowledgementWithoutAUsableCount(_ string: String) throws {
        let url = try #require(URL(string: string))

        #expect(AppMigrationLink.completedServerCount(from: url) == nil)
    }
