...
struct DomainTimestampStateTests {
    ...
    @Test func timestampSensorRendersRelativeTimeInsteadOfRawISOString() throws {
        ...
        #expect(description == RelativeDateTimeFormatter().localizedString(
            for: Self.referenceDate,
            relativeTo: Self.referenceDate.addingTimeInterval(-3600)
        ).leadingCapitalized)
    }
    ...
    @Test func dateSensorRendersLocalizedDateWithoutShiftingTheDay() throws {
        ...
        #expect(description == with(DateFormatter()) {
            $0.dateStyle = .medium
            $0.timeStyle = .none
            $0.timeZone = TimeZone(secondsFromGMT: 0)
        }.string(from: Self.referenceDate))
    }
