...
class SecurityExceptionsTests: XCTestCase {
    ...
    func testChallengeEvaluation() throws {
        ...
        let evaluate1 = exceptions.evaluate(authenticationChallenge(for: unitTestDotExampleDotCom1))
        XCTAssertEqual(evaluate1.0, .useCredential)
        XCTAssertNotNil(evaluate1.1)
        XCTAssertTrue(SecTrustEvaluateWithError(unitTestDotExampleDotCom1, nil))

        let evaluate2 = exceptions.evaluate(authenticationChallenge(for: unitTestDotExampleDotCom2))
        XCTAssertEqual(evaluate2.0, .useCredential)
        XCTAssertNotNil(evaluate2.1)
        XCTAssertTrue(SecTrustEvaluateWithError(unitTestDotExampleDotCom2, nil))

        let evaluate3 = exceptions.evaluate(authenticationChallenge(for: unitTestDotExampleDotCom3))
        XCTAssertEqual(evaluate3.0, .cancelAuthenticationChallenge)
    ...
    private func authenticationChallenge(for trust: SecTrust) -> URLAuthenticationChallenge {
        URLAuthenticationChallenge(
            protectionSpace: {
                ...
                space.perform(Selector(("_setServerTrust:")), with: trust)
                return space
            }(),
            proposedCredential: nil,
            previousFailureCount: 0,
...
private class FailingURLAuthenticationChallengeSender: NSObject, URLAuthenticationChallengeSender {
