...
class PromiseWebhookJsonTests: XCTestCase {
    ...
    func testUnencryptedNilData() {
        ...
        let json = promise.webhookJson(server: ServerFixture.standard, requestURL: nil, secretGetter: { nil })
    ...
    func testEncryptedEmptyData() throws {
        ...
        let json = promise.webhookJson(server: ServerFixture.standard, requestURL: nil, secretGetter: { Self.secret })
        XCTAssertNotNil(try hang(json))
    }
