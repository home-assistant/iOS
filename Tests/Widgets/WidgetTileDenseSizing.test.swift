...
struct WidgetTileDenseSizingTests {
    ...
    @Test func aRowGetsWhatThePaddingAndTheGapsLeave() {
        #expect(WidgetTileLayout.tileHeight(inGridOfHeight: 310, rows: 5) == (310 - 16 - 32) / 5)
        #expect(WidgetTileLayout.tileHeight(inGridOfHeight: 160, rows: 2) == (160 - 16 - 8) / 2)
        // A single row pays for no gaps.
        #expect(WidgetTileLayout.tileHeight(inGridOfHeight: 160, rows: 1) == 160 - 16)
    }
