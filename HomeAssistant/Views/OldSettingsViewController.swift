//
//  OldSettingsViewController.swift
//  HomeAssistant
//
...
class OldSettingsViewController: FormViewController, CLLocationManagerDelegate, SFSafariViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        center.addObserver(self,
                           selector: #selector(OldSettingsViewController.PermissionDidChange(_:)),
        ...
        let aboutButton = UIBarButtonItem(title: L10n.Settings.NavigationBar.AboutButton.title,
                                          style: .plain,
                                          target: self,
                                          action: #selector(OldSettingsViewController.openAbout(_:)))
        ...
        form
            ...
            <<< LabelRow("ssid") {
                ...
                if let ssids = self.internalBaseURLSSIDs {
                    $0.value = ssids.first
                } else if let ssid = ConnectionInfo.CurrentWiFiSSID {
                    $0.value = ssid
                }
                self.internalBaseURLSSIDs = [$0.value!]
            }
    ...
    private func configureDiscoveryObservers() {
        ...
        let center = NotificationCenter.default
        center.addObserver(self,
                           selector: #selector(OldSettingsViewController.HomeAssistantDiscovered(_:)),
                           name: NSNotification.Name(rawValue: "homeassistant.discovered"),
                           object: nil)

        center.addObserver(self,
                           selector: #selector(OldSettingsViewController.HomeAssistantUndiscovered(_:)),
                           name: NSNotification.Name(rawValue: "homeassistant.undiscovered"),
                           object: nil)

        center.addObserver(self,
                           selector: #selector(OldSettingsViewController.Connected(_:)),
